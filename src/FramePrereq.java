...
public class FramePrereq {

	private JFrame frameprereq;
	...
	private Image prereq_logo = new ImageIcon(FrameDashboard.class.getResource("res/PrereqLogo.png")).getImage().getScaledInstance(80,80,Image.SCALE_SMOOTH);
	private JTable table;
	private JComboBox<String> comboBoxcourseid ;
	private JComboBox<String> comboBoxprereqid ;
	private JTextField jtxtcourseid;
	private JTextField jtxtprereqid;

	/**
	 * Launch the application.
	 */
	public static void PrereqScreen() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					FramePrereq window = new FramePrereq();
					window.frameprereq.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	public void updateTable()
	{
		...
		if(conn != null)
		{
			String sql = "Select course_id, prereq_id  from prereq ";
			try 
			{
				...
				while(rs.next())
				{
					...
					columnData[1] = rs.getString("prereq_id");
	...
	private void updateComboprereqid() {
		String sql = "select * from course ";
		try {
			pst= conn.prepareStatement(sql);
			rs = pst.executeQuery();
	...
	public FramePrereq() {
		initialize();
		conn = Sqliteconnect.ConnectDB();
		Object col[] = {"course_id" , "prereq_id"};
	...
	private void initialize() {
		frameprereq = new JFrame();
		frameprereq.getContentPane().setBackground(new Color(70, 130, 180));
		frameprereq.setBounds(100, 100, 700, 700);
		frameprereq.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frameprereq.getContentPane().setLayout(null);
	
		contentPane = new JPanel();
		contentPane.setBackground(new Color(128, 128, 128));
		contentPane.setBorder(new LineBorder(new Color(70, 130, 180), 2));
		frameprereq.setContentPane(contentPane);
		...
		JLabel lblClassLogo = new JLabel("Prereq's Database : ");
		...
		lblClassLogo.setIcon(new ImageIcon(prereq_logo));
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					D[0] = Integer.parseInt(jtxtcourseid.getText()) ;
					D[1] = Integer.parseInt(jtxtprereqid.getText()) ;
				...
				try{
					...
					pst.setString(2,jtxtprereqid.getText());
		...
		btnDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String sql = "DELETE FROM prereq where course_id='"+cell+"'";
				try {
					pst = conn.prepareStatement(sql);
					pst.executeUpdate();
		...
		txtpncourseid.setBounds(10, 80, 183, 28);
		panelConfig.add(txtpncourseid);
		
		JTextPane txtpnprereqid = new JTextPane();
		txtpnprereqid.setEditable(false);
		txtpnprereqid.setText("Prereq ID : ");
		txtpnprereqid.setFont(new Font("Arial Black", Font.PLAIN, 15));
		txtpnprereqid.setBackground(Color.GRAY);
		txtpnprereqid.setBounds(10, 277, 183, 28);
		panelConfig.add(txtpnprereqid);
		
		jtxtprereqid = new JTextField();
		jtxtprereqid.setEditable(false);
		jtxtprereqid.setBounds(10, 344, 143, 28);
		panelConfig.add(jtxtprereqid);
		jtxtprereqid.setColumns(10);
		
		jtxtcourseid = new JTextField();
		jtxtcourseid.setEditable(false);
		jtxtcourseid.setBounds(10, 147, 143, 28);
		panelConfig.add(jtxtcourseid);
		jtxtcourseid.setColumns(10);
		
		comboBoxprereqid = new JComboBox<String>();
		comboBoxprereqid.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String selectedvalue = comboBoxprereqid.getSelectedItem().toString();
				jtxtprereqid.setText(selectedvalue);
				
			}
		});
		comboBoxprereqid.setBounds(10, 316, 143, 28);
		panelConfig.add(comboBoxprereqid);
		...
		comboBoxcourseid.setBounds(10, 119, 143, 28);
